...
module.exports = {
    name: 'setup-logging',
    description: 'Setup logging system in the server.',
    ...
    run: async(client, message, args) => {
        ...
        collector.on('end', async (collected, reason) => {
            if(reason === "0") {
                return message.channel.send({content: `${emojis.error} • Process has been stopped!`})
            }
            if(reason === "1") {
               return message.channel.send({content: `${emojis.error} • There was an error with your anwser, please make sure to follow the steps!`})
            }
            if(reason === "4") {
                await guilds.findOneAndUpdate({guildId: message.guild.id}, {
                    logging: false
                })
                return message.channel.send({content: `${emojis.tick} • Logging have now been disabled!`})
            ...
            if(reason === "2") {
                ...
                return message.channel.send({content: `${emojis.tick} • Logging data has now been setup!`})
            }
        })
    }
}